...
# From: https://github.com/kevalmorabia97/pyTweetCleaner
...
class TweetCleaner:
    ...
    def remove_hyperlinks(self, text):
        ...
        """
        return ' '.join([w for w in text.split(' ') if 'http' not in w])
    ...
    def get_cleaned_text(self, text):
        ...
        if re.match(r'RT @[_A-Za-z0-9]+:', cleaned_text):  # retweet
            ...
            retweet_info = cleaned_text[:cleaned_text.index(':') + 2]
        ...
        # remove stopwords and single length words
        tokens = [w for w in tokens if not w.lower() in self.stop_words and len(w) > 1]
    ...
    def get_cleaned_tweet(self, tweet):
        ...
        if not tweet['in_reply_to_status_id'] is None or not tweet['in_reply_to_user_id'] is None:
            # remove comments of any tweet
            return None
    ...
    def clean_tweets(self, input_file, output_file='cleaned_tweets.json'):
        ...
        while True:
            ...
            if cleaned_tweet is None: continue
            ...
            if 'retweeted_status' in tweet:  # will be present if it is a retweet
                ...
                if cleaned_tweet['retweeted_status'] is None: continue
...
if __name__ == '__main__':
    sample_text = 'RT @testUser: Cleaning unnecessary data with pyTweetCleaner by @kevalMorabia97. #pyTWEETCleaner, ' \
                  'Check it out at https:\\/\\/github.com\\/kevalmorabia97\\/pyTweetCleaner and star the repo!'

    tc = TweetCleaner(remove_stop_words=False, remove_retweets=False)
    tc.clean_tweets(input_file='data/sample_input.json',
                    output_file='data/sample_output.json')  # clean tweets from entire file
